  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <el-form :inline="true">
        ...
        <el-form-item>
          <el-select v-model="search.ticket_id" placeholder="请选择" style="width:300px;">
    ...
    <div v-loading="loading">
      <el-table :data="list" style="width: 100%;" @sort-change="sortChange" :default-sort="{prop: 'create_time', order: 'descending'}">
        ...
        <el-table-column prop="product_name" label="产品" width="200"></el-table-column>
...
export default {
  ...
  computed: {
    ...
    searchProductList() {
      return [
        ...
        ..._.map(this.productList, o => ({ ...o, name: `${o.source_name}-${o.name}` })),
      ];
    },
  },
  watch: {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/m/product_ticket_order', { params }));
  ...
  async mounted() {
    const { ticket_id } = this.$route.query;
    if (ticket_id) {
      this.search = {
        ...this.search,
