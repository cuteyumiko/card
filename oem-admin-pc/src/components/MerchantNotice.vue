  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加公告</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
...
export default {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/m/merchant_notice', { params }));
    ...
    showForm(item) {
      ...
      this.form = _.pick(item, ['id', 'name', 'icon', 'type_id', 'content', 'vip_content', 'view_count']);
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/merchant_notice/${id}`, form);
        } else {
          await this.$http.post('/api/merchant_notice', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/merchant_notice/${id}`);
        await this.reload();
  ...
  async mounted() {
    this.typeList = (await this.$http.get('/api/merchant_notice_type')).body;
    await this.reload();
  },
};
</script>
