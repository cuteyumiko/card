  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <el-form :inline="true">
        ...
        <el-form-item>
          <el-select v-model="search.loan_id" placeholder="请选择">
...
export default {
  data() {
    return {
      search: { current: 1, size: 10, status: 2, product_is_recommend: 1 },
      list: [],
      total: 0,
      loading: false,
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/m/product_loan_order', { params }));
  ...
  async mounted() {
    ...
    let { loan_id } = this.$route.query;
    if (loan_id) {
      loan_id = parseInt(loan_id, 10);
      const product = _.find(this.productList, { id: loan_id });
      this.search = {
        ...this.search,
        loan_id,
        product_is_recommend: product.is_recommend,
      };
    }
    await this.reload();
  },
