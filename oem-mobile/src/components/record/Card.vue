      ...
      <div style="display:flex;box-shadow:0px 0px 10px #eee;margin:5px;padding:5px;align-items:center;" v-for="o in showList" :key="o.id">
        ...
        <div style="flex:1">
          <div style="font-size:0.4rem;">
            ...
            <span v-if="o.status === 4" style="font-size:0.3rem;border-radius:10px;padding:2px 5px;border:1px solid red;color:red;float:right;margin-right:10px;">待首刷</span>
          </div>
...
export default {
  ...
  async mounted() {
    document.title = '办卡申请记录';
    let list = (await this.$http.get('/api/my/product_card_order_income?order=create_time-')).body;
    list = _.map(list, o => ({
      ...o,
    }));
    this.list = list;
