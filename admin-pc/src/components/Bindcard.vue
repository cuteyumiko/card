  ...
  <div style="padding:10px;display:flex:1;">
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加绑卡方式</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
...
export default {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/bindcard', { params }));
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/bindcard/${id}`, form);
        } else {
          await this.$http.post('/api/bindcard', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/bindcard/${id}`);
        await this.reload();
  ...
  async mounted() {
    this.passList = (await this.$http.get('/api/pass/bindcard')).body;
    await this.reload();
  },
};
</script>
