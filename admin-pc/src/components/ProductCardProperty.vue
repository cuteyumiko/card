  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        ...
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加信用卡属性</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}信用卡属性`" :visible.sync="formVisible" :close-on-click-modal="false">
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/v2/product_card_property', { params }));
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/v2/product_card_property/${id}`, form);
        } else {
          await this.$http.post('/api/v2/product_card_property', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/v2/product_card_property/${id}`);
        await this.reload();
