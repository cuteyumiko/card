    ...
    <el-form :inline="true">
      ...
      <el-form-item>
        <el-select v-model="search.card_id" placeholder="请选择">
          <el-option v-for="o in searchProductList" :key="o.id" :label="o.name" :value="o.id" filterable></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
        ...
        <el-table-column label="操作" width="240">
          <template slot-scope="{row}">
            <template v-if="row.status === 1">
              ...
              <el-button icon="el-icon-delete" size="mini" type="text" @click="updateItemStatus(row, 4)">待首刷</el-button>
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/product_card_order', { params }));
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/product_card_order/${id}`);
        await this.reload();
    ...
    async updateItemStatus({ id }, status) {
      ...
      try {
        await this.$http.put(`/api/product_card_order/${id}/status`, { status });
        await this.reload();
    ...
    async handleUpdateSelection() {
      ...
      for (let i = 0; i < this.selection.length; i += 1) {
        ...
        try {
          await this.$http.put(`/api/product_card_order/${id}/status`, { status: 3 });
        } catch (e) {
          const message = e.bodyText || e.message;
          this.$message.error(message);
        }
