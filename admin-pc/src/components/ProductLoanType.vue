  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-d-arrow-left" type="primary" size="small" @click="$router.push('/product_loan')">返回贷款</el-button>
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加贷款类型</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <div v-loading="loading">
      <el-table :data="list" style="width: 100%;">
        ...
        <el-table-column prop="catagory_id" label="大类"></el-table-column>
        <el-table-column prop="parent_name" label="父类"></el-table-column>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}贷款类型`" :visible.sync="formVisible" :close-on-click-modal="false">
      <el-form class="edit-form" :model="form" ref="form">
        <el-row :gutter="10">
          ...
          <el-col :span="8">
            <el-form-item label="大类（数字）">
              <el-input v-model="form.catagory_id" auto-complete="off"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="父类型">
              <el-select v-model="form.parent_id" placeholder="请选择" clearable style="width:100%;">
          ...
          <el-col :span="8">
            <el-form-item label="颜色">
              <el-color-picker v-model="form.color" style="width:100%;"></el-color-picker>
            </el-form-item>
          </el-col>
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/product_loan_type', { params }));
    ...
    showForm(item) {
      ...
      this.form = {
        ..._.pick(item, ['id', 'name', 'icon', 'code', 'catagory_id', 'parent_id', 'color']),
      };
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/product_loan_type/${id}`, form);
        } else {
          await this.$http.post('/api/product_loan_type', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/product_loan_type/${id}`);
        await this.reload();
