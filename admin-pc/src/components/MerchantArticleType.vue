  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/merchant_article_type', { params }));
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/merchant_article_type/${id}`, form);
        } else {
          await this.$http.post('/api/merchant_article_type', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/merchant_article_type/${id}`);
        await this.reload();
