  ...
  <div style="padding:10px;display:flex:1;">
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加积分券处理</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}积分处理`" :visible.sync="formVisible" :close-on-click-modal="false" width="80%">
...
export default {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/product_ticket_process', { params }));
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/product_ticket_process/${id}`, form);
        } else {
          await this.$http.post('/api/product_ticket_process', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/product_ticket_process/${id}`);
        await this.reload();
  ...
  async mounted() {
    this.passList = (await this.$http.get('/api/pass/product_ticket_process')).body;
    await this.reload();
  },
};
</script>
