  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-d-arrow-left" type="primary" size="small" @click="$router.push('/course')">返回课程</el-button>
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加类型</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}课程类型`" :visible.sync="formVisible" :close-on-click-modal="false">
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/course_type', { params }));
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/course_type/${id}`, form);
        } else {
          await this.$http.post('/api/course_type', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/course_type/${id}`);
        await this.reload();
