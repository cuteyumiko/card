  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加信用卡</el-button>
        <el-button icon="el-icon-menu" type="primary" size="small" @click="$router.push('/product_card_type')">查看类型</el-button>
        <el-button icon="el-icon-menu" type="primary" size="small" @click="$router.push('/product_card_source')">查看来源</el-button>
        <el-button icon="el-icon-menu" type="primary" size="small" @click="$router.push('/product_card_property')">信用卡属性</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}信用卡`" :visible.sync="formVisible" :close-on-click-modal="false">
            ...
            <el-row :gutter="10">
              ...
              <el-col :span="24">
                <el-form-item label="描述">
                  <el-input v-model="form.description" type="textarea" :rows="2" auto-complete="off"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
              ...
              <el-col :span="8">
                <el-form-item label="图标2">
                  <el-upload class="avatar-uploader"
                    action="/api/upload"
                    :show-file-list="false"
                    :on-success="handleSuccess_icon2">
                    <img v-if="form.icon2" :src="form.icon2" class="avatar">
                    <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                  </el-upload>
                </el-form-item>
              </el-col>
...
export default {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/product_card', { params }));
    ...
    async showForm({ id } = { }) {
      ...
      const { property, ...item } = id ? (await this.$http.get(`/api/v2/product_card/${id}?prop=property`)).body : { property: [] };
      const type = item.type_ids ? _.map(item.type_ids.replace(/\[|\]/g, '').split(','), p => _.find(this.typeList, q => `${q.id}` === p)) : item.type_ids;
      this.form = {
        ..._.pick(item, ['id', 'name', 'type_id', 'description', 'got_count', 'icon', 'icon2', 'href', 'recommend_bg', 'apply_bg', 'qr_rect', 'text_color', 'sort', 'source_id', 'code', 'tip_text', 'money']),
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/v2/product_card/${id}`, form);
        } else {
          await this.$http.post('/api/v2/product_card', form);
        }
    ...
    async updateItem(id, form) {
      try {
        await this.$http.patch(`/api/product_card/${id}`, form);
      } catch (e) {
        const message = e.bodyText || e.message;
        this.$message.error(message);
      }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/product_card/${id}`);
        await this.reload();
  ...
  async mounted() {
    this.typeList = (await this.$http.get('/api/product_card_type')).body;
    this.sourceList = (await this.$http.get('/api/product_card_source')).body;

    this.propertyList = _.map((await this.$http.get('/api/v2/product_card_property')).body, (o) => {
      const ext = {};
      if (o.type_id === 3) {
        ext.handleUpload = () => {
