  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        ...
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加信用卡类型</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}信用卡类型`" :visible.sync="formVisible" :close-on-click-modal="false">
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/product_card_type', { params }));
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/product_card_type/${id}`, form);
        } else {
          await this.$http.post('/api/product_card_type', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/product_card_type/${id}`);
        await this.reload();
