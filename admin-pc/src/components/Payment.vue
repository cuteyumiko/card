  ...
  <div style="padding:10px;display:flex:1;">
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加支付方式</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <div v-loading="loading">
      <el-table :data="list" style="width: 100%;">
        ...
        <el-table-column label="微信中" width="100">
          <template slot-scope="{row}">
            <el-switch @change="updateItem(row.id, { used_weixin: row.used_weixin })" v-model="row.used_weixin"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="手机浏览器中" width="100">
          <template slot-scope="{row}">
            <el-switch @change="updateItem(row.id, { used_mobile_browser: row.used_mobile_browser })" v-model="row.used_mobile_browser"></el-switch>
          </template>
        </el-table-column>
        <el-table-column label="APP中" width="100">
          <template slot-scope="{row}">
            <el-switch @change="updateItem(row.id, { used_app: row.used_app })" v-model="row.used_app"></el-switch>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}支付方式`" :visible.sync="formVisible" :close-on-click-modal="false" width="80%">
      <el-form class="edit-form" :model="form" ref="form">
        <el-row :gutter="10">
          ...
          <el-col :span="24" v-if="passByCode[form.pass_code]">
...
export default {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/payment', { params }));

        this.list = _.map(body, o => ({
          ...o,
          used_weixin: !!o.used_weixin,
          used_mobile_browser: !!o.used_mobile_browser,
          used_app: !!o.used_app,
    ...
    showForm(item) {
      ...
      const { pass_config, ...form } = _.pick(item, ['id', 'name', 'description', 'pass_code', 'pass_config', 'icon']);
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/payment/${id}`, form);
        } else {
          await this.$http.post('/api/payment', form);
        }
  ...
  async mounted() {
    this.passList = (await this.$http.get('/api/pass/payment')).body;
    await this.reload();
  },
};
</script>
