  ...
  <div style="padding:10px;display:flex:1;">
    <div style="display:flex;">
      <div style="margin-right:10px;">
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加代付方式</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <div v-loading="loading">
      <el-table :data="list" style="width: 100%;">
        ...
        <el-table-column prop="is_enabled" label="启用">
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}代付方式`" :visible.sync="formVisible" :close-on-click-modal="false" width="80%">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="8">
            <el-form-item label="启用">
              <el-switch v-model="form.is_enabled" style="width:100%;"></el-switch>
            </el-form-item>
          </el-col>
          <el-col :span="24">
...
export default {
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/agentpay', { params }));
    ...
    showForm(item) {
      ...
      const { pass_config, ...form } = _.pick(item, ['id', 'name', 'code', 'pass_code', 'pass_config', 'is_enabled']);
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/agentpay/${id}`, form);
        } else {
          await this.$http.post('/api/agentpay', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/agentpay/${id}`);
        await this.reload();
  ...
  async mounted() {
    this.passList = (await this.$http.get('/api/pass/agentpay')).body;
    await this.reload();
  },
};
</script>
