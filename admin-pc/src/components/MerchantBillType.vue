  ...
  <div style="padding:10px;display:flex:1;">
    <div style="display:flex;">
      <div style="margin-right:10px;">
        ...
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加海报类型</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
    ...
    <el-dialog :title="`${form.id ? '编辑' : '添加'}海报类型`" :visible.sync="formVisible" :close-on-click-modal="false" width="80%">
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/merchant_bill_type', { params }));
    ...
    showForm(item) {
      ...
      const { pass_config, ...form } = _.pick(item, ['id', 'name', 'code']);
      this.form = form;
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/merchant_bill_type/${id}`, form);
        } else {
          await this.$http.post('/api/merchant_bill_type', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/merchant_bill_type/${id}`);
        await this.reload();
