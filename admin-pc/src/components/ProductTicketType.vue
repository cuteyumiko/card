  ...
  <div style="padding:10px;display:flex:1;">
    ...
    <div style="display:flex;">
      <div style="margin-right:10px;">
        ...
        <el-button icon="el-icon-plus" type="primary" size="small" @click="showForm()">添加积分券类型</el-button>
      </div>
      <el-form :inline="true">
        <el-form-item>
  ...
  methods: {
    reload: _.throttle(async function _reload() {
      ...
      try {
        ...
        params.limit = size;
        const { body, headers } = (await this.$http.get('/api/product_ticket_type', { params }));
    ...
    showForm(item) {
      ...
      this.form = {
        ..._.pick(item, ['id', 'name', 'icon', 'description']),
      };
    ...
    async saveForm() {
      ...
      try {
        ...
        if (id) {
          await this.$http.patch(`/api/product_ticket_type/${id}`, form);
        } else {
          await this.$http.post('/api/product_ticket_type', form);
        }
    ...
    async handleDelete({ id, name }) {
      ...
      try {
        await this.$http.delete(`/api/product_ticket_type/${id}`);
        await this.reload();
