...
class ExportComponent extends Component {
  ...
  render() {
    ...
    if(!item) return null;
    return (
      <TouchableOpacity onPress={() => this.props.navigation.navigate('cardApply', { id: item.id})} style={{flexDirection:'row', padding: 10, alignItems:'center'}}>

        <Image source={ item.icon ? {uri: item.icon} : require('../assets/icon-180.png') } style={{width:60, height:60}} />
        <View style={{marginLeft:13, flex:1}}>
          <View style={{flexDirection: 'row', alignItems:'center'}}>
            <Text style={{fontSize:15,color:'#333333'}}>{item.name} </Text><View style={{backgroundColor:'#fde0d8', paddingVertical:1, paddingHorizontal:3, borderRadius:2}}><Text style={{fontSize:10, color:'#ED471C'}}>{item.tip_text}</Text></View>
          </View>
  ...
  async componentDidMount() {
    ...
    const [item] = (await axios.post('/api/v2/object', [
      JSON.stringify({ object: 'product_card_with_merchant', method: 'get', query: { id, merchant_id: this.props.merchant.id, prop: 'property' }})
    ])).data;
