...
class Screen extends Component {
  ...
  render() {
    return (
      <View style={{flex: 1, backgroundColor:'#fff'}}>
        ...
        {this.state.item ? (
          <ScrollView>
            <QRCodeImage ref={ref => this.qr = ref} width="100%" source={{uri: this.state.item.recommend_bg}} qr={{value:`${axios.defaults.baseURL}/product_loan_apply?referee=${this.props.user.id}&id=${this.state.item.id}`, rect: this.state.item.qr_rect}} text={`推荐人：${this.props.user.nickname}`} textColor={this.state.item.text_color} />
  ...
  async componentDidMount() {
    ...
    const item = (await axios.get(`/api/m/product_loan/${id}`)).data;
    this.setState({item});
  }
}
