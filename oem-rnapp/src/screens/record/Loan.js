...
class Screen extends Component {
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            <View style={{marginBottom: 10, paddingBottom: 10, backgroundColor:'#ffffff', borderTopLeftRadius: 6}}>
              <View style={{flexDirection:'row', paddingTop:18, paddingLeft: 15, paddingRight: 15, borderLeftWidth:6, borderColor: '#FFC573', borderTopLeftRadius: 6}}>
                ...
                {item.status === 3 ? (
                  ...
                  <Text style={{fontSize:14, color:'#CAA473'}}>{(item.status === 1 || item.status === 4) ? '预计佣金' : '结算佣金'}
                    <Text style={{fontSize:26, color:'#FF300F'}}>
                      {(item.money_unit == '％' && item.order_money) ? (item.money * item.order_money / 100) : item.money }
                    </Text><Text style={{fontSize:16, color:'#FF300F'}}>{ (item.money_unit == '％' && item.order_money) ? '元' : item.money_unit}</Text>
                  </Text>
                )}
  ...
  async reload(refresh){
    ...
    const resp_list = (await axios.get('/api/my/product_loan_order_income', { params: {
      order: 'create_time',
